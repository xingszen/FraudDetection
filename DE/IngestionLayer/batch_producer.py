...
class BatchProcuder:
    def __init__(self, hdfs_base_dir: str):
        """Initializes the HDFS Ingestor and ensures the base directory exists."""
        self.hdfs_base_dir = hdfs_base_dir
        self._ensure_hdfs_directory(self.hdfs_base_dir)

    def _run_cmd(self, cmd: list) -> bool:
        """Helper method to execute shell commands securely."""
        try:
            # capture_output=True prevents Hadoop's Java logs from polluting our clean Python logs
    ...
    def _ensure_hdfs_directory(self, hdfs_path: str):
        """Creates the HDFS directory idempotently."""
        logger.info(f"Ensuring HDFS directory exists: {hdfs_path}")
        # -p flag ensures no error is thrown if the directory already exists (idempotency)
    ...
    def ingest_file(self, local_file_path: str, hdfs_target_name: str = None):
        ...
        # This makes the ingestion script idempotent and safe to re-run.
    ...
    def _verify_upload(self, hdfs_path: str):
        """Verifies the file size in HDFS to ensure complete transfer."""
        cmd = ["hdfs", "dfs", "-ls", hdfs_path]
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            # Parse the ls output to log the file metadata
...
if __name__ == "__main__":
    # Define paths
    LOCAL_CSV_FILE = "data/sample_100000_sorted.csv"  # The large dataset for batch
