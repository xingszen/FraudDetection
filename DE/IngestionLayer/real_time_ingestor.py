...
class RealTimeIngestor:
    ...
    def _create_topic_if_missing(self, topic_name: str, num_partitions: int, replication_factor: int):
        """Uses KafkaAdminClient to create topics idempotently."""
        admin_client = None
        try:
...
if __name__ == "__main__":
    ...
    # Gets the directory where this script lives, then goes up one level to 'DE'
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    basic_csv_path = os.path.join(base_dir, 'data', '5000_New_Basic.csv')
    advanced_csv_path = os.path.join(base_dir, 'data', '5000_New_Advanced.csv')

    # 1. Initialize Producer for Topic A
    streamer_a = RealTimeIngestor(
        bootstrap_servers=KAFKA_BROKER, 
        topic='financial-transactions-v22', 
        num_partitions=3, 
    ...
    streamer_b = RealTimeIngestor(
        bootstrap_servers=KAFKA_BROKER, 
        topic='ip-account-detection-v22', 
        num_partitions=3, 
        replication_factor=1
    )
    ...
    # 5. Wait until both threads are completely finished
    thread_a.join()
    thread_b.join()
